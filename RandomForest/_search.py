...
from _validation import check_scorer, fit_and_score, \
    ...
    def fit(self, x, y=None, **fit_params):
        ...
        **fit_params : dict of string -> object
        ...
        def evaluate_candidates(candidate_params):
            ...
            validation_data = []
            fits = []
            for parameters, (train, validation) in product(candidate_params, cv.split(x, y)):
                validation_data.append(validation)
                fits.append(fit(clone(base_estimator), train, scorer=scorers, parameters=parameters, 
                                  fit_params=fit_params))
            out = [score_func(estimator, validation, scorer=scorers) for estimator, validation 
in zip(fits, validation_data)]
            nonlocal n_splits
